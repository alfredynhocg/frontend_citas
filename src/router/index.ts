...
const router = createRouter({
  ...
  routes: [
    {
      path: '/',
      redirect: '/mordern-auth/login'
    },
    // -- 100 Citas: rutas propias --
    {
      path: '/dashboard/citas',
      name: 'Dashboard100Citas',
      meta: { title: 'Dashboard — 100 Citas' },
      component: () => import('@/views/citas/dashboard/index.vue'),
    },
    {
      path: '/citas',
      name: 'Citas',
      meta: { title: 'Catálogo de Citas' },
      component: () => import('@/views/citas/catalogo/index.vue'),
    },
    {
      path: '/parejas',
      name: 'Parejas',
    ...
    },
    {
      ...
      component: () => import('@/views/citas/recuerdos/index.vue'),
    },
    {
      path: '/perfil',
      name: 'Perfil',
...
const PUBLIC_PATHS = new Set([
  '/mordern-auth/login',  '/mordern-auth/register',
  '/basic-auth/login',   '/basic-auth/register',
  '/cover-auth/login',   '/cover-auth/register',
  '/boxed-auth/login',   '/boxed-auth/register',
  '/maintenance', '/offline', '/error', '/coming-soon',
])
...
router.beforeEach((to) => {
  ...
  // Sesión expirada: viene con query ?expired=1
  if (to.path === '/mordern-auth/login' && to.query.expired) {
    return true
  }

  if (isPublic && auth.isAuthenticated) {
    return { path: '/dashboard/citas' }
  }

  if (!isPublic && !auth.isAuthenticated) {
    return { path: '/mordern-auth/login' }
  }
...
router.afterEach((to) => {
  ...
  document.title = title ? `${title} | 100 Citas` : '100 Citas Románticas'
})
